...
public class UserLoginControllerTest {
	...
	public void testUserLoginProcess_CorrectInfo_Succeed() throws Exception {
		RequestBuilder request = MockMvcRequestBuilders.post("/user/login/process")
				.param("userEmail", "test@example.com").param("userPassword", "password123");

		// ログインが成功したらリダイレクトされることと、セッションにユーザー情報が保存されていることをテスト
		mockMvc.perform(request).andExpect(view().name("redirect:/user/report/list"));
	}
	...
	public void testUserLoginProcess_IncorrectInfo_Fail() throws Exception {
		RequestBuilder request = MockMvcRequestBuilders.post("/user/login/process")
				.param("userEmail", "wrong@example.com").param("userPassword", "wrongpass");

		// ログインが失敗したらエラーメッセージが表示され、ログイン画面に留まることをテスト
		mockMvc.perform(request).andExpect(model().attribute("errorMessage", "メールアドレスまたはパスワードが正しくありません"))
